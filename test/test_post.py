...
@pytest.mark.parametrize("title, content, published",[
    ("new titlee", "new contentt", True),
    ("favorite pizza", "pepperoni", False),
    ("test data 3", "test content 3", True)
])
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    ...
    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "titletitle1"
    assert created_post.content == "contentcontent1"
...
def test_delete_non_exist(authorized_client, test_user, test_posts):
    res = authorized_client.delete(
        f"/posts/800000000000")
    assert res.status_code == 404
...
def test_update_non_exist(authorized_client, test_user, test_posts):
    ...
    res = authorized_client.put(
        f"/posts/800000000000", json=data)
    assert res.status_code == 404